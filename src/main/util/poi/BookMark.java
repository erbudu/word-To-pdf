...
public class BookMark {
	...
	/**
	 * 插入图片
	 * @param bookmarkValue 书签名
	 * @param map 文件
	 */
	void insertPhotoAtBookMark(String bookmarkValue, Map<String,String> map) {
		XWPFRun run;
		String big = "big";
		String small = "small";
		...
		String imgFilePath = map.get("filepath");

		try {
			FileInputStream fileInputImg = new FileInputStream(imgFilePath);
			BufferedImage bufferedImg = ImageIO.read(fileInputImg);
			...
			if(big.equals(map.get("type"))) {
				run.addPicture(fileInputImg,Document.PICTURE_TYPE_JPEG,
						bookmarkValue, Units.toEMU(400),Units.toEMU(((float)400/imgWidth)*imgHeight));
			}
			else if(small.equals(map.get("type"))) {
				run.addPicture(fileInputImg,Document.PICTURE_TYPE_JPEG,
						bookmarkValue, Units.toEMU(60),Units.toEMU(((float)60/imgWidth)*imgHeight));
				//获取图片数据
				CTDrawing drawing = run.getCTR().getDrawingArray(0);
				CTGraphicalObject graphicalobject = drawing.getInlineArray(0).getGraphic();
				//拿到新插入的图片替换添加CTAnchor 设置浮动属性 删除inline属性
				CTAnchor anchor = getAnchorWithGraphic(graphicalobject, bookmarkValue,
						Units.toEMU(63), Units.toEMU(((float)63/imgWidth)*imgHeight),
						Units.toEMU(78), Units.toEMU(1), true);
				//添加浮动属性
				drawing.setAnchorArray(new CTAnchor[]{anchor});
				//删除行内属性
				drawing.removeInline(0);
			}
			else if(middle.equals(map.get("type"))) {
				run.addPicture(fileInputImg,Document.PICTURE_TYPE_JPEG,
						bookmarkValue, Units.toEMU(130),Units.toEMU(((float)130/imgWidth)*imgHeight));
				//获取图片数据
				CTDrawing drawing = run.getCTR().getDrawingArray(0);
				CTGraphicalObject graphicalobject = drawing.getInlineArray(0).getGraphic();
				//拿到新插入的图片替换添加CTAnchor 设置浮动属性 删除inline属性
				CTAnchor anchor = getAnchorWithGraphic(graphicalobject, bookmarkValue,
						Units.toEMU(130), Units.toEMU(((float)130/imgWidth)*imgHeight),
						Units.toEMU(280), Units.toEMU(-38), true);
				//添加浮动属性
				drawing.setAnchorArray(new CTAnchor[]{anchor});
				//删除行内属性
				drawing.removeInline(0);
